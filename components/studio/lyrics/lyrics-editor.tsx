...
export function LyricsEditor() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (activeId && (e.key === 'Delete' || e.key === 'Backspace')) {
        e.preventDefault();
        e.stopPropagation();
        deleteLyric(activeId);
        setActiveId(null);
        return;
      }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.name.endsWith('.lrc')) setLyrics(parseLrcLyrics(text));
    else setLyrics(parseTxtLyrics(text, duration));
    setImportMode(null);
  ...
  const handleImport = () => {
    ...
    if (importMode === 'txt' || importMode === 'manual') setLyrics(parseTxtLyrics(pastedText, duration));
    else if (importMode === 'lrc') setLyrics(parseLrcLyrics(pastedText));
    setImportMode(null);
    setPastedText('');
  };
  ...
  const handleDrop = (id: string, e: React.DragEvent) => {
    ...
    for (let i = 0; i < steps; i++) moveLyric(dragId, direction);
    ...
    <div className="flex h-full bg-background">
      <div className="w-64 shrink-0 border-r border-border bg-card/30 flex flex-col">
        ...
        <div className="p-3 space-y-2">
          ...
          <Button variant="outline" size="sm" className="w-full justify-start gap-2" onClick={() => { setImportMode('txt'); setPastedText(''); }}>
            <FileText className="h-4 w-4" /> Pegar texto (TXT)
          </Button>
          <Button variant="outline" size="sm" className="w-full justify-start gap-2" onClick={() => { setImportMode('lrc'); setPastedText(''); }}>
            <FileText className="h-4 w-4" /> Pegar LRC con tiempos
          </Button>
          <Button variant="outline" size="sm" className="w-full justify-start gap-2" onClick={() => { setImportMode('manual'); setPastedText(''); }}>
            <Plus className="h-4 w-4" /> Editor manual
          </Button>
        </div>
          ...
          {!tapSyncActive ? (
            ...
            <div className="space-y-2">
              <div className="flex gap-1">
                {audio.isPlaying ? (
                  <Button size="sm" variant="outline" className="flex-1 gap-1" onClick={pauseTapSync}><Pause className="h-3.5 w-3.5" /> Pausar</Button>
                ) : (
                  <Button size="sm" variant="outline" className="flex-1 gap-1" onClick={resumeTapSync}><Play className="h-3.5 w-3.5" /> Reanudar</Button>
                )}
        ...
        <div className="p-3 space-y-2">
          ...
          <Button variant="outline" size="sm" className="w-full justify-start gap-2" onClick={() => setShiftOpen(!shiftOpen)}>
            <Clock className="h-4 w-4" /> Desplazar tiempos
          </Button>
          {shiftOpen && (
            <div className="space-y-2 p-2 rounded-md bg-secondary/30">
              <div className="flex items-center gap-1">
                <Input type="number" step="0.1" value={shiftValue} onChange={(e) => setShiftValue(parseFloat(e.target.value) || 0)} className="h-8 text-xs" placeholder="Segundos" />
                <Button size="sm" variant="outline" className="h-8 px-2" onClick={() => { shiftTimestamps(shiftValue); setShiftValue(0); }}>Aplicar</Button>
              </div>
              <p className="text-[10px] text-muted-foreground">Positivo = adelante, negativo = atrás</p>
            </div>
          )}
          <Button variant="outline" size="sm" className="w-full justify-start gap-2" onClick={() => clearTimestamps()}>
            <RotateCcw className="h-4 w-4" /> Borrar tiempos
          </Button>
        </div>
        ...
        <div className="p-3 space-y-2">
          ...
          <Button variant="outline" size="sm" className="w-full justify-start gap-2" onClick={handleExportTxt} disabled={lyrics.length === 0}>
            <Download className="h-4 w-4" /> Exportar TXT
          </Button>
          <Button variant="outline" size="sm" className="w-full justify-start gap-2" onClick={handleExportLrc} disabled={lyrics.length === 0}>
            <Download className="h-4 w-4" /> Exportar LRC
          </Button>
        </div>
        ...
        {importMode ? (
          <div className="flex-1 flex flex-col p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setImportMode(null)}><X className="h-4 w-4" /></Button>
            </div>
            <p className="text-sm text-muted-foreground mb-4">{importMode === 'lrc' ? 'Formato: [mm:ss.xx] texto de la línea' : 'Una línea por cada frase de la letra'}</p>
            <Textarea autoFocus value={pastedText} onChange={(e) => setPastedText(e.target.value)} placeholder={importMode === 'lrc' ? '[00:01.00] Primera línea\n[00:05.20] Segunda línea' : 'Escribe o pega la letra aquí...'} className="flex-1 font-mono text-sm resize-none" />
            <div className="flex justify-end gap-2 mt-4">
              <Button variant="outline" onClick={() => setImportMode(null)}>Cancelar</Button>
              <Button onClick={handleImport} disabled={!pastedText.trim()}><Check className="h-4 w-4 mr-1" /> Importar</Button>
            </div>
          </div>
        ) : (
          <>
            <div className="h-10 flex items-center justify-between px-4 border-b border-border">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button size="sm" variant="outline" onClick={() => addLyric()} className="gap-1.5 h-7"><Plus className="h-4 w-4" /> Añadir línea</Button>
              </div>
            </div>
            ...
            {searchOpen && (
              <div className="flex items-center gap-2 px-4 py-2 border-b border-border bg-card/50">
                ...
                <Input type="text" placeholder="Buscar..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-8 max-w-[200px]" autoFocus />
                <Replace className="h-4 w-4 text-muted-foreground shrink-0" />
                <Input type="text" placeholder="Reemplazar..." value={replaceQuery} onChange={(e) => setReplaceQuery(e.target.value)} className="h-8 max-w-[200px]" />
                <Button size="sm" variant="outline" className="h-8 text-xs" onClick={() => setMatchCase(!matchCase)}>{matchCase ? 'Aa' : 'aa'}</Button>
                <Button size="sm" variant="default" className="h-8 text-xs gap-1" onClick={handleReplace} disabled={!searchQuery}><Replace className="h-3.5 w-3.5" /> Reemplazar</Button>
                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setSearchOpen(false); setSearchQuery(''); setReplaceQuery(''); }}><X className="h-4 w-4" /></Button>
              </div>
            )}
                  ...
                  return (
                    <ContextMenu key={line.id}>
                      <ContextMenuTrigger asChild>
                        <div
                          ref={(el) => { if (el) itemRefs.current.set(line.id, el); }}
                          draggable
                          onDragStart={(e) => handleDragStart(line.id, e)}
                          onDragOver={(e) => handleDragOver(line.id, e)}
                          onDrop={(e) => handleDrop(line.id, e)}
                          onDragEnd={() => { dragIdRef.current = null; setDragOverId(null); }}
                          className={`flex items-center gap-2 p-2 rounded-lg border transition-all cursor-pointer ${isTapSyncCurrent ? 'border-primary bg-primary/15 ring-2 ring-primary/40' : isPlaying ? 'border-primary/60 bg-primary/10' : isActive ? 'border-primary bg-primary/10 ring-2 ring-primary/30' : isDragOver ? 'border-primary/50 bg-primary/5' : status === 'past' ? 'border-border/50 bg-muted/20 opacity-60' : status === 'upcoming' ? 'border-border bg-card/30' : 'border-border hover:border-primary/30'} ${matchesSearch ? 'ring-1 ring-yellow-500/40' : ''}`}
                        ...
                        >
                          ...
                          {editingId === line.id ? (
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') { updateLyric(line.id, { text: (e.target as HTMLInputElement).value }); setEditingId(null); }
                            ...
                            <span className={`flex-1 text-sm truncate ${status === 'past' ? 'text-muted-foreground' : 'text-foreground'} ${line.text === '' ? 'italic text-muted-foreground/50' : ''}`}>{line.text || 'Línea vacía'}</span>
                          )}

                          <div className="flex items-center gap-1 shrink-0">
                            <Input type="number" step="0.1" value={line.start > 0 ? line.start.toFixed(1) : ''} onChange={(e) => updateLyric(line.id, { start: parseFloat(e.target.value) || 0 })} onClick={(e) => e.stopPropagation()} className="w-16 h-7 text-[11px] font-mono" placeholder="Inicio" />
                            <span className="text-muted-foreground text-[10px]">→</span>
                            <Input type="number" step="0.1" value={line.end > 0 ? line.end.toFixed(1) : ''} onChange={(e) => updateLyric(line.id, { end: parseFloat(e.target.value) || 0 })} onClick={(e) => e.stopPropagation()} className="w-16 h-7 text-[11px] font-mono" placeholder="Fin" />
                          </div>

                          <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" title="Asignar tiempo actual" onClick={(e) => { e.stopPropagation(); handleSetTimeToLine(line.id); }}><Clock className="h-3.5 w-3.5" /></Button>
                          <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" title="Reproducir desde aquí" onClick={(e) => { e.stopPropagation(); if (line.start > 0) audio.seek(line.start); audio.play(); }}><Play className="h-3.5 w-3.5" /></Button>
                        </div>
                      </ContextMenuTrigger>
                      <ContextMenuContent>
                        <ContextMenuItem onClick={() => duplicateLyric(line.id)}><Copy className="h-4 w-4 mr-2" /> Duplicar</ContextMenuItem>
                        <ContextMenuItem onClick={() => moveLyric(line.id, 'up')}><ArrowUp className="h-4 w-4 mr-2" /> Mover arriba</ContextMenuItem>
                        <ContextMenuItem onClick={() => moveLyric(line.id, 'down')}><ArrowDown className="h-4 w-4 mr-2" /> Mover abajo</ContextMenuItem>
                        <ContextMenuItem onClick={() => handleSetTimeToLine(line.id)}><Clock className="h-4 w-4 mr-2" /> Asignar tiempo actual</ContextMenuItem>
                        <ContextMenuSeparator />
                        <ContextMenuItem className="text-destructive" onClick={() => { deleteLyric(line.id); if (activeId === line.id) setActiveId(null); }}><Trash2 className="h-4 w-4 mr-2" /> Eliminar</ContextMenuItem>
                      </ContextMenuContent>
                    </ContextMenu>
                  );
                })}
